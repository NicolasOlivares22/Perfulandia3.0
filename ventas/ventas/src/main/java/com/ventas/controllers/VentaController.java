...
public class VentaController {
    ...
    public VentaDTO obtenerHATEOAS(@PathVariable Integer id) {   //tuve que agregar un opcional en el obtener por id en el service 
        ...
        dto.add(Link.of("http://localhost:8092/api/proxy/ventas/" + dto.getIdVenta()).withSelfRel());
        dto.add(Link.of("http://localhost:8092/api/proxy/ventas/" + dto.getIdVenta())
    ...
    public List<VentaDTO> obtenerTodosHATEOAS() {
        ...
        for (VentaDTO dto : lista) {
            ...
            dto.add(Link.of("http://localhost:8092/api/proxy/ventas").withRel("Get todas HATEOAS"));
            dto.add(Link.of("http://localhost:8092/api/proxy/ventas/" + dto.getIdVenta())
