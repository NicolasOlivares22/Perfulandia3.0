...
public class InventarioController {
    ...
    public ResponseEntity<List<Inventario>> getAll() {
        List<Inventario> lista = inventarioService.findAll(); //tuve que cambiar el findById por findAll porque me dio problemas la id
...
//Hateoas
    ...
    public ResponseEntity<Inventario> obtenerHATEOAS(@PathVariable Integer id) {
        ...
        Link selfGateway = Link.of("http://localhost:8091/api/proxy/inventario/" + inv.getId_Inventario()).withSelfRel();
        Link putGateway = Link.of("http://localhost:8091/api/proxy/inventario/" + inv.getId_Inventario()).withRel("Actualizar HATEOAS").withType("PUT");
        Link deleteGateway = Link.of("http://localhost:8091/api/proxy/inventario/" + inv.getId_Inventario()).withRel("Eliminar HATEOAS").withType("DELETE");
    ...
    public ResponseEntity<List<Inventario>> obtenerTodosHATEOAS() {
        ...
        for (Inventario inv : lista) {
            ...
            Link selfLink = linkTo(methodOn(InventarioController.class).obtenerHATEOAS(id)).withSelfRel();
            Link crearLink = Link.of("http://localhost:8091/api/proxy/inventario/" + id).withRel("Crear HATEOAS").withType("POST");
